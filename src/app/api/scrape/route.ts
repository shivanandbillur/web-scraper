    ...
    async start(controller) {
      ...
      try {
        ...
        while (collectedLeads.length < targetCount) {
          ...
          const prompt = `
Generate EXACTLY 15 highly diverse LinkedIn X-Ray search strings for the following user request.
USER QUERY: "${query.replace(/"/g, '')}"

STRATEGY THIS ROUND: "${strategyKey}"
(Rotate your boolean logic based on the strategy. Use advanced operators like OR, AND, -, (), quotas.)

RULES:
- Extract any target location, industry, or roles from the user query.
- Only output LinkedIn profile URLs formats like site:linkedin.com/in/ OR site:linkedin.com/pub/
- Make sure to use synonyms for the jobs/roles mentioned.
- Output ONLY a raw JSON array of 15 strings.

ALREADY SEARCHED (avoid these exact formulas to ensure we get fresh leads):
${recentForAI}
          ...
          const searchTasks = newQueries.map(async (searchTerm, queryIndex) => {
            ...
            while (collectedLeads.length < targetCount) {
              ...
              for (const lead of results) {
                ...
                // ICP FILTER - Make it generic based on user string negation
                let status = "KEPT";
                let reason = "ICP Match";
                
                // If user specifies explicit anti-keywords (e.g. "-agency"), we catch that
                const cleanUserQuery = query.toLowerCase();
                const negativeKeywords = cleanUserQuery.split(' ').filter((w: string) => w.startsWith('-')).map((w: string) => w.substring(1));
                
                for (const neg of negativeKeywords) {
                  if (neg.length > 2 && fullText.includes(neg)) {
                     status = "REJECTED";
                     reason = `Negative keyword matched: ${neg}`;
                     break;
                  }
                }
                ...
                try {
                  db.prepare(`
                    VALUES (?, ?, ?, ?, ?, ?)
                  `).run(cleanUrl, lead.name, lead.title, lead.company || "Unknown", "Global", lead.bio);
                  isFresh = true;
                } catch (err: any) {
                ...
                try {
                  const csvLine = `"${safeString(lead.name)}","${safeString(lead.title)}","","","","Global","${safeString(lead.company || "Unknown")}","","Global","","${lead.url}","","${dateStr}","AI WebAgent","Automated X-Ray","","","",\n`;
